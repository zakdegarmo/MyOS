...
NLBL_VALUE = 4 # <--- Defines the base for this specific script
...
# For NLBL4, digits are '1', '2', '3', '4'.
BASE_DIGITS = [str(i) for i in range(1, NLBL_VALUE + 1)] # Digits 1 to NLBL_VALUE
RECURSIVE_DEPTH = NLBL_VALUE # Depth is equal to the NLBL_VALUE (4 for NLBL4)
...
def create_nested_folders(current_path, current_depth, max_depth, base_digits):
    """
    Recursively creates nested folders (1,2,3,4) up to a specified depth.
    """
    if current_depth >= max_depth: # Stop when max_depth is reached
        return

    for digit in base_digits:
        folder_name = digit # Folder names are simply the digits (1, 2, 3, 4)
        ...
        try:
            ...
            # Removed os.path.relpath here because current_path is already absolute in this context
...
def main():
    ...
    # Get the list of items at the root of SANDBOX_BASE_PATH (excluding '.' '..' and '0' folder itself)
    # These are the 1,2,3,4 folders that were initially created as top-level.
    items_to_move = [
