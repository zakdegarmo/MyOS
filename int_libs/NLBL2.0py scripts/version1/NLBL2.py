...
NLBL_VALUE = 2 # <--- NEW: Defines the base for this specific script
...
# For NLBL2, digits are '1', '2'.
BASE_DIGITS = [str(i) for i in range(1, NLBL_VALUE + 1)] 
RECURSIVE_DEPTH = NLBL_VALUE # Depth is equal to the NLBL_VALUE (2 for NLBL2)
...
def create_nested_folders(current_path, current_depth, max_depth, base_digits):
    ...
    for digit in base_digits:
        folder_name = digit # Folder names are simply the digits (1, 2, etc.)
        ...
        try:
            ...
            print(f"ERROR: Could not create nested folder '{os.path.relpath(folder_path, SANDBOX_BASE_PATH)}'. Reason: {e}")
...
def main():
    ...
    # This creates the top-level 1, 2 (for NLBL2) folders inside '0'
    for digit in BASE_DIGITS:
    ...
    # This step is implicitly handled by creating the fractal *inside* the '0' folder from the start.
